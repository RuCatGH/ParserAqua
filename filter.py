...
def getUrls(): # Получение списка страниц
    ...
    for filter in filters:
        ...
        filterUrl = filtersPage[filter].replace('-', '=', 1).replace('cgrs','cid') # Дастаём ключ из словаря со всеми фильтрами и заменяем на знак = (для пост запроса)
